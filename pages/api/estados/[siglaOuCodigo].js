...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('Resposta da API IBGE (getUfByCode):', response.status, response.data);
    ...
    console.log('Resposta da API IBGE (metadados):', metadadosResponse.status, metadadosResponse.data);

    if (metadadosResponse.status !== 200 || !Array.isArray(metadadosResponse.data)) {
    ...
    return res.status(500).json({ error: err.message || 'Erro ao buscar estado.' });
  }
}
