    ...
    expect(data).toEqual(
      expect.objectContaining({
        ...
        authors: expect.arrayContaining([
          expect.stringMatching(/katsuhiro/i),
        ]),
...
describeIfCbl('api/isbn/v1 (E2E) > Provider cbl', () => {
  test('Utilizando provider cbl', async () => {
    const response = await axios.get(
      `${BASE_ISBN_URL()}/9788545702870?providers=cbl`
    );
    const { data, status } = response;
    ...
    expect(data).toEqual(
      expect.objectContaining({
        ...
        synopsis: expect.stringContaining(
          'Um dos marcos da ficção científica'
        ),
        dimensions: expect.objectContaining({
          width: 17.5,
          height: 25.7,
...
describeIfMercadoEditorial('api/isbn/v1 (E2E) > Provider mercado-editorial', () => {
  test('Utilizando provider mercado-editorial', async () => {
    const response = await axios.get(
      `${BASE_ISBN_URL()}/9786555140576?providers=mercado-editorial`
    );
    const { data, status } = response;
...
describeIfOpenLibrary('api/isbn/v1 (E2E) > Provider open-library', () => {
  test('Utilizando provider open-library', async () => {
    const response = await axios.get(
      `${BASE_ISBN_URL()}/9788545702870?providers=open-library`
    );
    const { data, status } = response;
...
describeIfGoogleBooks('api/isbn/v1 (E2E) > Provider google-books', () => {
  test('Utilizando provider google-books', async () => {
    const response = await axios.get(
      `${BASE_ISBN_URL()}/9788545712466?providers=google-books`
    );
    const { data, status } = response;
    ...
    expect(data).toEqual(
      expect.objectContaining({
        ...
        synopsis: expect.stringContaining(
          'Punpun Onodera é um garoto normal'
        ),
        dimensions: null,
        year: 2019,
        format: 'DIGITAL',
